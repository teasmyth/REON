...
OctreeNode::OctreeNode(const FBox& Bounds, const float& MinSize, OctreeNode* Parent, int& ID)
{
	...
	ChildrenNodeBounds[0] = FBox(NodeBounds.Min, Center);
	ChildrenNodeBounds[1] = FBox(FVector());
	ChildrenNodeBounds[2] = FBox();
	ChildrenNodeBounds[3] = FBox();

	ChildrenNodeBounds[4] = FBox();
	ChildrenNodeBounds[5] = FBox();
	ChildrenNodeBounds[6] = FBox();
	ChildrenNodeBounds[7] = FBox();
